...
static Value *EmitTargetArchBuiltinExpr(CodeGenFunction *CGF,
  ...
  case llvm::Triple::shuxi32:
    return CGF->EmitSHUXIBuiltinExpr(BuiltinID, E);
...
Value *CodeGenFunction::EmitSHUXIBuiltinExpr(unsigned BuiltinID,
                                             const CallExpr *E) {
  switch (BuiltinID)
  {
  case SHUXI::BI__shuxi_test_i:
  {
    llvm::Value *Src0 = EmitScalarExpr(E->getArg(0));
    llvm::Value *Src1 = EmitScalarExpr(E->getArg(1));
    llvm::Type *SrcType = Src0->getType();

    Value *F = CGM.getIntrinsic(Intrinsic::shuxi_test_i, { SrcType, SrcType, SrcType });
    return Builder.CreateCall(F, { Src0, Src1 });
  }

  default:
    assert("Unsupported builtin functions");
    return nullptr;
  }
}
